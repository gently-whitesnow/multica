...
External integrations use the immutable projection contract rather than the
ordinary member issue command:

```bash
MULTICA_TOKEN=msp_... MULTICA_WORKSPACE_ID=<workspace-uuid> \
  multica issue external create --provider overtime --instance-id primary \
  --external-id task-1842 --title "Implement invoice export"

multica issue external get overtime primary task-1842
```

An identical create retry prints the original issue and link. A changed payload
for the same external key fails with `409`.
...
| | `external create/get` | Create-or-get an issue by immutable external reference | `create`: `--provider`, `--instance-id`, `--external-id`, `--title`, optional backlink/assignee/project; `get`: provider, instance ID, external ID |
