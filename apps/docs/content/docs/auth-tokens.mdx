...
The server also recognizes these credentials used in internal, managed, or
...
| `msp_` | [Service principals](/service-principals) — scoped machine identity for an external integration | A workspace owner or admin |

`mcn_` and `mdt_` are issued by the platform itself; regular Cloud, self-hosted, and Desktop installations never need to create them manually. `msp_` is the only one an admin creates on purpose, and only for an external integration. The user-side CLI and daemon always use `mul_` PATs; don't construct tokens with other prefixes yourself.
