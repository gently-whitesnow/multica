...
## Default boundary: the daemon user
...
Linux operators that need the task and control plane to have different OS
identities can opt into the built-in fail-closed executor with
`MULTICA_LINUX_TASK_ISOLATION=required`. It creates a fresh system user for
each task, gives that user only the current managed task root and a new `HOME`,
and removes the user after success, failure, cancellation, or timeout. A root
supervisor also watches the daemon PID and kills every process with the task
UID if the daemon crashes. If identity creation, isolated launch, or cleanup
fails, the daemon reports task failure; it never retries as the daemon user.

This mode deliberately rejects `local_directory` resources: granting an
ephemeral task identity access to an operator-owned checkout would break the
boundary. See the fork's [Linux deployment contract](https://github.com/gently-whitesnow/multica/blob/main/docs/linux-task-isolation.md) for the sudo rule, permissions, credential model, and runtime test.
...
- **Your `HOME` directory layout on the default path.** Tasks inherit the daemon user's real `HOME` and `XDG_*` variables, which is what makes host CLIs (`gh`, `aws`, `kubectl`, `gcloud`, `glab`) work inside a task exactly as they do in your shell. It also means everything under that home is reachable. Required Linux task isolation replaces those paths with a task-owned home and does not project daemon files into it.
